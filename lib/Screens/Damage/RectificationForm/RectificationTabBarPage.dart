...
import 'package:ecm_v2/Screens/Damage/RectificationForm/Devices/OMS.dart';
import 'package:ecm_v2/Widgets/CustomAppBar.dart';
import 'package:ecm_v2/Widgets/POP-Ups/ChangeLanguage.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';

class RectificationTabBarTool extends StatefulWidget {
  const RectificationTabBarTool({super.key});

  @override
  State<RectificationTabBarTool> createState() =>
      _RectificationTabBarToolState();
}

class _RectificationTabBarToolState extends State<RectificationTabBarTool> {
  ...
  void _initTabs() {
    ...
    final pages = [OmsRectification(), AmsDamage(), RmsDamage(), LoRaDamage()];
    ...
    final positions = <int>[];
    final drString = ap.selectedProject?.drString ?? "";

    for (int i = 0; i < drString.length; i++) {
      if (drString[i] == '1' && i < tabNames.length && i < pages.length) {
        positions.add(i);
